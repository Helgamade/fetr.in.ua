...
### Автоматический деплой (через Git hook post-receive):

**Git hook автоматически выполняется после `git push origin main`**

Автоматически выполняется:
1. `git fetch origin && git reset --hard origin/main` - обновление кода
2. `npm run build` - сборка фронтенда
3. **КРИТИЧНО:** Копирование файлов из `dist/` в корень:
   - `cp dist/index.html index.html` (ПЕРВЫМ!)
   - `cp -r dist/assets/* assets/`
   - Копирование других файлов (favicon.ico, robots.txt)
4. Установка прав доступа:
   - `chmod 755 assets`
   - `chmod 644 assets/*`
   - `chmod 644 index.html`
5. Перезапуск сервера: `pkill -f "node.*index.js" && nohup node index.js &`

### Ручной деплой (через скрипт deploy.sh):

Если автоматический деплой не сработал, используй скрипт:
...
**Скрипт выполняет те же действия:**
1. `npm run build` - сборка фронтенда
