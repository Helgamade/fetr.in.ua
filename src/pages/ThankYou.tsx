      ...
      <div className={`min-h-screen bg-gradient-to-b ${isPaymentPending ? 'from-yellow-50' : 'from-green-50'} to-background`}>
        ...
        <div className="container mx-auto px-4 py-8 max-w-2xl">
          ...
          <div className="bg-card rounded-2xl p-6 shadow-soft mb-8">
            ...
            {(() => {
              // Определяем способ доставки и статус оплаты
              const deliveryMethod = order?.delivery?.method;
              const paymentMethod = order?.payment?.method;
              const paymentStatus = order?.payment?.status;
              const isPaid = paymentStatus === 'paid';
              const isDelivery = deliveryMethod === 'nova_poshta' || deliveryMethod === 'ukrposhta';
              const isPickup = deliveryMethod === 'pickup';

              // Формируем список пунктов в зависимости от доставки, оплаты и статуса оплаты
              let steps: string[] = [];
              ...
              if (isDelivery) {
                ...
                if (paymentMethod === 'wayforpay') {
                  if (isPaid) {
                    // Оплачено - без пункта про оплату
                    steps = [
                      'Очікуйте на повідомлення від нашого менеджера',
                      'Отримайте ТТН для відстеження посилки',
                      'Заберіть замовлення у відділенні та творіть із задоволенням ✨'
                    ];
                  } else {
                    steps = [
                      'Оплатіть замовлення онлайн через WayForPay',
                      'Очікуйте на повідомлення від нашого менеджера',
                      'Отримайте ТТН для відстеження посилки',
                      'Заберіть замовлення у відділенні та творіть із задоволенням ✨'
                    ];
                  }
                } else if (paymentMethod === 'nalojka') {
                  if (isPaid) {
                    // Оплачено - без пункта про оплату
                    steps = [
                      'Очікуйте на повідомлення від нашого менеджера',
                      'Отримайте ТТН для відстеження посилки',
                      'Заберіть замовлення та творіть із задоволенням ✨'
                    ];
                  } else {
                    steps = [
                      'Очікуйте на повідомлення від нашого менеджера',
                      'Отримайте ТТН для відстеження посилки',
                      'Оплатіть замовлення при отриманні у відділенні',
                      'Заберіть замовлення та творіть із задоволенням ✨'
                    ];
                  }
                } else if (paymentMethod === 'fopiban') {
                  if (isPaid) {
                    // Оплачено - без пункта про оплату
                    steps = [
                      'Очікуйте на повідомлення від нашого менеджера',
                    ...
                    steps = [
                      'Оплатіть замовлення на рахунок ФОП (реквізити вказані вище)',
                      'Очікуйте на повідомлення від нашого менеджера',
                      'Отримайте ТТН для відстеження посилки',
                      'Заберіть замовлення та творіть із задоволенням ✨'
                    ];
                  ...
                  // Fallback для неизвестного способа оплаты
                  if (isPaid) {
                    steps = [
                      'Очікуйте на повідомлення від нашого менеджера',
                      'Отримайте ТТН для відстеження посилки',
                    ...
                    steps = [
                      'Оплатіть замовлення (якщо обрали передоплату)',
                      'Очікуйте на повідомлення від нашого менеджера',
                      'Отримайте ТТН для відстеження посилки',
                      'Заберіть замовлення та творіть із задоволенням ✨'
                    ];
                ...
                if (paymentMethod === 'wayforpay') {
                  ...
                  if (isPaid) {
                    ...
                    steps = [
                      'Очікуйте на повідомлення від нашого менеджера',
                      'Оплатіть замовлення готівкою при отриманні',
                      'Заберіть замовлення та творіть із задоволенням ✨'
                    ];
                  }
                } else if (paymentMethod === 'fopiban') {
                ...
                // Fallback для неизвестного способа доставки
                if (isPaid) {
                  steps = [
                    'Очікуйте на повідомлення від нашого менеджера',
                    'Отримайте ТТН для відстеження посилки',
